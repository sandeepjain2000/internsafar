...
export async function GET(request) {
  ...
  const result = await query(
    `SELECT id, table_key, name, filters, sort, is_default, created_at, updated_at
     FROM ip_list_presets
...
export async function POST(request) {
  ...
  const count = await query(
    `SELECT count(*)::int AS n FROM ip_list_presets WHERE user_id = $1 AND table_key = $2`,
    [session.user.id, tableKey],
  );
  ...
  try {
    await query(
      `INSERT INTO ip_list_presets (id, user_id, table_key, name, filters, sort, is_default, updated_at)
    ...
    if (String(err?.message || '').includes('ip_list_presets') && String(err?.message || '').includes('unique')) {
  ...
  if (body.isDefault) {
    await query(
      `UPDATE ip_list_presets SET is_default = (id = $3) WHERE user_id = $1 AND table_key = $2`,
...
export async function PATCH(request) {
  ...
  const row = await query(
    `SELECT id, table_key, is_default FROM ip_list_presets WHERE id = $1 AND user_id = $2`,
    [id, session.user.id],
  );
  ...
  if (typeof body.isDefault === 'boolean') {
    if (body.isDefault) {
      await query(
        `UPDATE ip_list_presets SET is_default = (id = $3), updated_at = now()
         WHERE user_id = $1 AND table_key = $2`,
        [session.user.id, row.rows[0].table_key, id],
      );
    } else {
      await query(
        `UPDATE ip_list_presets SET is_default = false, updated_at = now() WHERE id = $1 AND user_id = $2`,
        [id, session.user.id],
      );
    }
  }
...
export async function DELETE(request) {
  ...
  if (!id) return jsonError('id required');
  await query(`DELETE FROM ip_list_presets WHERE id = $1 AND user_id = $2`, [id, session.user.id]);
  return jsonOk({ ok: true });
}
