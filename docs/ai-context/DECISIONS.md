...
| Profile/registration label = **Country**; list filters label = **Region**. Shared options in `src/lib/ipRegions.js`: India, Pakistan, Bangladesh, Sri Lanka, Nepal, Indonesia, Malaysia, Thailand. Candidate browse filters employer `hq_country`; employer search filters candidate `country` via `region` query. Country/Region pickers use SearchableMultiSelect (profile stores one country; filters allow multi) | Candidate / Employer | Confirmed | `ipRegions.js`, migration `042_*`, browse + candidates + profile pages |
## Do not add here
