/**
 * Comprehensive headless Playwright + API QA for all 179 cases in
 * test-cases/Internship_Portal_Test_Checklist.xlsx.
 *
 * Usage:
 *   node scripts/run-ip-checklist-qa.mjs [baseUrl]
 *   node scripts/run-ip-checklist-qa.mjs --apply [baseUrl]
 *
...
async function runApiSuite() {
  ...
  try {
    ...
    if (!enabledAfter) {
      if (!ENABLE_CODE || ENABLE_CODE.length !== 6) blocked('AUTH-12', 'Need IP_QA_2FA_ENABLE_CODE (6 digits) from Zoho');
      else blocked('AUTH-12', '2FA enable confirmation failed (check enable OTP)');
    } else {
      // Confirm that login requires OTP (challengeId present) even if we don't have the OTP code yet.
      ...
      if (!login1.otpRequiredChallengeId) {
        ...
        blocked('AUTH-12', '2FA enabled but IP_QA_2FA_LOGIN_CODE (6 digits) not provided');
      } else {
    ...
    // AUTH-19: start-enable when already enabled should be rejected (no OTP needed)
    if (enabledAfter) {
    ...
    // AUTH-14: disable if code provided
    if (DISABLE_CODE && DISABLE_CODE.length === 6) {
      ...
      if (!disChallengeId) {
        ...
        const enabledAfter = await api2faGetEnabled(cand.cookie);
        assess('AUTH-14', disConfirm.status === 200 && !enabledAfter.enabled, {
          confirmStatus: disConfirm.status,
          enabledAfter: enabledAfter.enabled,
        });
      }
    } else {
      blocked('AUTH-14', 'Need IP_QA_2FA_DISABLE_CODE (6 digits) from Zoho');
    }
  ...
  const regEmpMismatch = await api('/api/ip/auth/register-employer', {
    method: 'POST',
    body: { email: 'person@gmail.com', website: 'https://example.com', companyName: 'Mismatch Co', captchaToken: 'x', captchaAnswer: '7', mode: 'auto' },
  });
  ...
  const regEmpNoWeb = await api('/api/ip/auth/register-employer', {
    method: 'POST',
    body: { email: 'person@example.com', companyName: 'No Website', captchaToken: 'x', captchaAnswer: '7', mode: 'auto' },
  });
  ...
  // SuperAdmin APIs
  const saStats = await api('/api/ip/superadmin/stats', { cookie: sa.cookie });
  assess('SA-D-1', saStats.status === 200, saStats.data);

  const saPostings = await api('/api/ip/superadmin/postings', { cookie: sa.cookie });
  assess('SA-PO-1', saPostings.status === 200,
    { count: (saPostings.data?.items || saPostings.data || []).length });

  const saDocs = await api('/api/ip/superadmin/documents', { cookie: sa.cookie });
  assess('SA-DOC-1', saDocs.status === 200,
    { count: (saDocs.data?.documents || saDocs.data?.items || saDocs.data || []).length });

  const saLoginRep = await api('/api/ip/superadmin/login-report', { cookie: sa.cookie });
  assess('SA-L-1', saLoginRep.status === 200, { status: saLoginRep.status });

  const saEmp = await api('/api/ip/superadmin/employers', { cookie: sa.cookie });
  assess('SA-A-1', saEmp.status === 200,
    { count: (saEmp.data?.employers || saEmp.data?.items || []).length });

  const saReqs = await api('/api/ip/superadmin/requests', { cookie: sa.cookie });
  ...
  // Message thread tenancy — we cannot easily verify cross-tenant without second cast user,
  // mark as blocked
  blocked('CAND-M-5', 'Needs two distinct cast candidate accounts sharing a thread — manual or future fixture');
  blocked('CAND-O-7', 'Needs a real offer id belonging to a different candidate — cannot generate without fixture');
  blocked('EMP-I-9', 'Needs two approved employer accounts; mark manual');

  // Double-publish not testable without a real posting
  blocked('EMP-I-10', 'Needs existing published employer listing — manual verification');
  blocked('EMP-I-11', 'Needs existing listing and points balance — manual verification');

  // Bootstrap — just check SA can log in (tested via AUTH-1 equivalent)
...
async function runBrowserSuite(logins) {
  ...
  try {
    ...
    assess('AUTH-15',
      await visible(page, 'input[type="email"], #email'),
      { url: page.url() });
    ...
    // PERM-2: candidate trying employer shell
    const empWrongRole = await fetchRaw('/employer', {
      redirect: 'manual',
      cookie: logins.cand.cookie,
    });
    assess('PERM-2',
      empWrongRole.status >= 300 && empWrongRole.status < 400,
    ...
    // Blocked — things that need special setup / DB fixtures
    blocked('AUTH-3', 'Empty email/password: captcha bypass mode makes empty-submit test ambiguous; manual only');
    blocked('AUTH-5', 'Session TTL 12h vs 30d requires time-travel; manual');
    blocked('AUTH-7', 'Requires pending/inactive/rejected cast account; manual setup');
    blocked('AUTH-8', 'Requires DB outage simulation; manual');
    // AUTH-11/12/13/14/19 are handled in the API suite when optional OTP env vars are provided.
    blocked('AUTH-17', 'Requires temp new password + reversal; manual to avoid locking cast account');
    blocked('AUTH-22', 'Login event verified by SA-L-1 API check + manual inspection');
    blocked('REG-C-1', 'Unique Gmail needed; use manual QA inbox test');
    blocked('REG-C-4', 'Form candidate pending path requires manual inspection of SA queue');
    blocked('REG-C-6', 'Referral credit requires new unique Gmail registration; manual');
    blocked('REG-C-7', 'Form-path referral approval flow; manual');
    blocked('REG-C-8', 'Googlemail.com alias hard to mint in automated test');
    blocked('REG-C-9', 'Self-referral needs careful email/code combo; manual');
    blocked('REG-C-11', 'Captcha-before-insert path blocked by CAPTCHA_BYPASS_FOR_TESTING');
    blocked('REG-E-1', 'Domain employer creation with unique matching domain; manual');
    blocked('REG-E-4', 'Manual request path; needs fresh email; manual');
    blocked('REG-E-6', 'Duplicate employer domain email needs unique matching domain; manual');
    blocked('ACCT-4', 'Requires spare Gmail for email change; manual');
    blocked('ACCT-5', 'Requires phone verify inbox; manual');
    blocked('ACCT-3', 'Notification preference PUT side-effects (email suppression) need fixture; API shape verified');
    blocked('CAND-B-2', 'Filter permutations need seeded internships; semi-manual');
    blocked('CAND-B-5', 'Incomplete-profile apply gate; needs cast account with profile_complete=false');
    blocked('CAND-B-6', 'Remote vs location filter needs seeded remote internship');
    blocked('CAND-B-7', 'Work-mode alias normalisation; needs specific seed posting');
    blocked('CAND-A-1', 'Happy-path apply needs fresh published internship + points; manual for point spend');
    blocked('CAND-A-2', 'Duplicate apply needs prior apply fixture');
    blocked('CAND-A-3', 'Apply to draft ID needs employer draft id; manual');
    blocked('CAND-A-4', 'Screening questions need seeded posting with questions');
    blocked('CAND-A-5', 'Zero-balance account not available in cast set');
    blocked('CAND-A-6', 'Match score advisory needs apply fixture + skills comparison');
    blocked('CAND-M-4', 'Empty message reply needs existing thread; manual');
    blocked('CAND-O-2', 'Decline offer needs pending offer fixture');
    blocked('CAND-O-3', 'Expired offer state needs offer with past validUntil');
    blocked('EMP-I-1', 'Draft posting save needs approved employer with profile; semi-manual');
    blocked('EMP-I-2', 'Publish flow needs 50 points + approved profile; manual spend test');
    blocked('EMP-I-3', 'Zero-point employer needed; manual');
    blocked('EMP-I-4', 'Needs cast pending employer account');
    blocked('EMP-I-5', 'Needs approved employer with profile_complete false');
    blocked('EMP-I-7', 'Edit/close needs existing posting id; manual');
    blocked('EMP-I-8', 'Form chrome check; manual UI walk');
    blocked('EMP-PL-1', 'Full pipeline status cycle needs applicant fixture');
    blocked('EMP-PL-3', 'Create offer from applicant needs applicant fixture');
    blocked('EMP-PL-5', 'Completion + ratings unlock needs hired fixture');
    blocked('EMP-PL-7', 'Invalid Meet URL on interviewing needs existing application');
    blocked('EMP-P-3', 'Ethics checklist enforcement needs employer profile fixture');
    blocked('EMP-AN-1', 'Analytics page requires at least one posting/applicant; manual');
    blocked('EMP-C-2', 'Search invite modal chrome; manual UI walk');
    blocked('EMP-M-1', 'Thread compose needs cast employer + candidate pair');
    blocked('EMP-R-1', 'Employer referral credit needs new domain employer registration');
    blocked('SA-F-2', 'Reject form candidate needs pending fixture + login check');
    blocked('SA-F-3', 'Bulk form reg action needs pending fixtures');
    blocked('SA-A-2', 'Reject/suspend employer cycle needs approved employer fixture');
    blocked('SA-R-2', 'Reject manual request needs pending request fixture');
    blocked('SA-R-3', 'Duplicate request create needs processed request fixture');
    blocked('SA-L-2', 'Login report table chrome; manual');
    blocked('IDEA-1', 'Submit idea needs category id (may work; semi-manual)');
    blocked('IDEA-2', 'Vote/follow/comment needs existing idea id');
    blocked('RATE-1', 'Ratings after completion need full hire→complete fixture');
    blocked('PTS-2', 'Convert endpoint status; manual API check');
    blocked('FILE-1', 'Upload photo/resume needs multipart fixture');
    blocked('MAIL-1', 'Mail override verification needs QA inbox inspection; manual');
    blocked('REGX-2', 'Table isolation needs DB observer; manual');
    blocked('OFF-R-1', 'Offer remind needs pending offer fixture');
...
async function main() {
  console.log(`IP comprehensive checklist QA — base=${BASE} headless=true`);
  ...
  const passN = Object.values(cases).filter((c) => c.status === 'Pass').length;
  const failN = Object.values(cases).filter((c) => c.status === 'Fail').length;
  const blockedN = Object.values(cases).filter((c) => c.status === 'Blocked').length;
  const total = Object.keys(cases).length;

  console.log(JSON.stringify({ executedAt, base: BASE, total, pass: passN, fail: failN, blocked: blockedN, resultsFile: 'test-cases/qa-results.json' }));
